...
class DACServer( LabradServer ):
    ...
    def setMultipoleControlFile(self, c, CfilePath):                
        ...
        self.multipoleMatrix = {elec: {mult: data[int(elec) + index*len(hc.elecDict) - 1] for index, mult in enumerate(hc.multipoles)} for elec in hc.elecDict.keys()}
    ...
    def setMultipoleValues(self, c, ms):
        ...
        for e in hc.elecDict.keys():
            ...
            for n in range(self.numCols):
                for m in hc.multipoles: voltageMatrix[e][n] += self.multipoleMatrix[e][m][n] * self.multipoleSet[m]
        if self.numCols > 1:
